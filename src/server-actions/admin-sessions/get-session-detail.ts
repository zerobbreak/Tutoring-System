...
import type { SessionAttendanceRowDTO } from "#/server-actions/lecturer-sessions/types";
import { assertModuleInInstitution } from "./helpers";
import { claimIdSchema } from "./schemas";
import type { AdminSessionDetailDTO, AdminSessionDisputeDTO } from "./types";
...
export const getAdminSessionDetailFn = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<AdminSessionDetailDTO> => {
    ...
    if (!module) throw new Error("Session not found.");
    await assertModuleInInstitution(supabase, module.id, institutionId);
    ...
    const evidence: AdminSessionDetailDTO["evidence"] = [];
    ...
    const { data: attRows, error: attErr } = await supabase
      ...
      .select("id, student_id, status, check_in_time, is_verified, notes")
    ...
    const { data: disputeRows, error: dErr } = await supabase
      .from("disputes")
      .select("id, status, reason, raised_at, resolution_note")
      .eq("claim_id", data.claimId)
      .order("raised_at", { ascending: false });

    if (dErr) throw new Error(dErr.message);

    const disputes: AdminSessionDisputeDTO[] = (disputeRows ?? []).map((d) => ({
      id: d.id as string,
      status: d.status as string,
      reason: d.reason as string,
      raised_at: d.raised_at as string,
      resolution_note: (d.resolution_note as string | null) ?? null,
    }));

    const open_dispute = disputes.some((d) => d.status === "OPEN");
    ...
    const qrToken = claim.qr_token as string | null;
    ...
    return {
      ...
      qr_check_in_url:
        qrToken != null
          ? `/student/check-in?session=${data.claimId}&token=${qrToken}`
          : null,
      module: unwrapOne(
        claim.module as
          | { id: string; code: string; name: string }
