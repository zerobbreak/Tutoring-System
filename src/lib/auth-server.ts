 ...
 */
export const signUpServerFn = createServerFn("POST", async (data: any) => {
  ...
  if (role === "lecturer" || role === "tutor" || role === "admin") {
    if (verificationCode !== VERIFICATION_CODES[role as keyof typeof VERIFICATION_CODES]) {
  ...
  return { 
    success: true, 
    user: authData.user ? JSON.parse(JSON.stringify(authData.user)) : null 
  };
});
 ...
 */
export const signInServerFn = createServerFn("POST", async (data: any) => {
  ...
  const userPOJO = authData.user ? JSON.parse(JSON.stringify(authData.user)) : null;
 ...
 */
export const updateProfileServerFn = createServerFn("POST", async (data: { fullName: string }) => {
  const { fullName } = data;
